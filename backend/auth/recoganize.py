...
def AuthenticateFace():
    ...
    faceCascade = cv2.CascadeClassifier(cascadePath) # type: ignore

    font = cv2.FONT_HERSHEY_SIMPLEX  # type: ignore # denotes the font type
    ...
    cam = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # type: ignore # cv2.CAP_DSHOW to remove warning
    ...
    while True:
        ...
        converted_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # type: ignore
        ...
        for(x, y, w, h) in faces:
            ...
            cv2.putText(img, str(id), (x+5, y-5), font, 1, (255, 255, 255), 2) # type: ignore
            cv2.putText(img, str(accuracy), (x+5, y+h-5), # type: ignore
                        font, 1, (255, 255, 0), 1)

        cv2.imshow('camera', img) # type: ignore
